<template>
  <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
    <SectionLabel>OCM (OpenAI Codex Multiplexer)</SectionLabel>
  </div>
  <div class="grid2">
    <Field :label="$t('types.ocm.credentialPath')">
      <input class="input mono" v-model="data.credential_path" />
    </Field>
    <Field :label="$t('types.ocm.usagesPath')">
  ...
  <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
    <SectionLabel>{{ $t('types.ocm.users') }}</SectionLabel>
  ...
  >
    ...
    <div class="grid2">
      <Field :label="$t('types.ocm.userName')" :mb="0">
        <input class="input mono" v-model="user.name" />
      </Field>
      <Field :label="$t('types.ocm.userToken')" :mb="0">
